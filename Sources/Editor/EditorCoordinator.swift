...
final class EditorCoordinator: BonsplitDelegate, @unchecked Sendable {
    ...
    private func createEditorState(for tab: TabData) -> EditorState {
        ...
        let statusLine = state.vimStatusLineView
        state.textView.onVimCommandLineUpdate = { [weak statusLine] text in
            guard let statusLine else { return }
        ...
        state.textView.onVimCommandExecute = { [weak self] command, forceIt in
            guard let self else { return false }
            return self.handleVimCommand(command, forceIt: forceIt)
        }
    ...
    func saveFileAs() {
        guard let selectedTabStoreID = selectedTabStoreID() else { return }
        let previousURL = tabStore.tabs.first(where: { $0.id == selectedTabStoreID })?.fileURL
        tabStore.saveFileAs(id: selectedTabStoreID)
        if let bonsplitID = tabIDMap[selectedTabStoreID],
           let tab = tabStore.tabs.first(where: { $0.id == selectedTabStoreID }) {
            controller.updateTab(bonsplitID, title: tab.name, isDirty: tab.isDirty)
            if previousURL != tab.fileURL, let fileURL = tab.fileURL {
                if let previousURL {
                    fileWatcher.stop(url: previousURL)
                }
    ...
    @MainActor
    func closeCurrentTab(force: Bool) {
        guard let focusedPaneId = controller.focusedPaneId,
        ...
        _ = closeTab(tabID: selectedTab.id, force: force)
    }

    @MainActor
    private func closeTab(tabID: TabID, force: Bool, skipConfirmation: Bool = false) -> Bool {
        guard let tabStoreID = reverseMap[tabID],
              let tab = tabStore.tabs.first(where: { $0.id == tabStoreID }) else { return false }

        if !skipConfirmation && !force && !confirmCloseTab(tab) { return false }
        if let state = editorStates[tabID] {
            tabStore.updateCursorPosition(id: tabStoreID, position: state.textView.selectedRange().location)
        }
        isClosingConfirmedTab = true
        let didClose = controller.closeTab(tabID)
        isClosingConfirmedTab = false
        return didClose
    }

    @MainActor
    private func closeAllTabs(force: Bool) -> Bool {
        let closableTabIDs = controller.allTabIds.filter { $0 != clipboardTabID }

        if !force {
            for tabID in closableTabIDs {
                guard let tabStoreID = reverseMap[tabID],
                      let tab = tabStore.tabs.first(where: { $0.id == tabStoreID }) else { return false }
        ...
        for tabID in closableTabIDs {
            if !closeTab(tabID: tabID, force: force, skipConfirmation: true) {
                return false
            }
        }
        return true
    ...
    @MainActor
    func handleVimCommand(_ command: String, forceIt: Bool) -> Bool {
        let cleanCommand = command.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !cleanCommand.isEmpty else { return false }

        let parts = cleanCommand.split(whereSeparator: { $0.isWhitespace })
        guard let raw = parts.first else { return false }

        var commandName = raw.lowercased()
        var isForce = forceIt

        if commandName.hasSuffix("!") && commandName.count > 1 {
            isForce = true
            commandName.removeLast()
        }
        ...
        case "w", "write":
            ...
            return true
        case "q", "quit":
            closeCurrentTab(force: isForce)
            return true
        case "x", "exi", "exit", "xit":
            saveFile()
            closeCurrentTab(force: isForce)
            return true
        case "qa", "qall", "quitall":
            return closeAllTabs(force: isForce)
        case "wq":
            saveFile()
            closeCurrentTab(force: isForce)
            return true
    ...
    private func saveAllTabs() {
        for tabID in tabStore.tabs.map(\.id) {
            if clipboardTabID != nil, let bonsplitID = tabIDMap[tabID], bonsplitID == clipboardTabID {
                continue
            }
            guard tabStore.tabs.first(where: { $0.id == tabID })?.fileURL != nil else { continue }
            tabStore.saveFile(id: tabID)
            if let bonsplitID = tabIDMap[tabID], let tab = tabStore.tabs.first(where: { $0.id == tabID }) {
                controller.updateTab(bonsplitID, title: tab.name, isDirty: tab.isDirty)
            }
        }
    }
    ...
    private func selectedTabStoreID() -> UUID? {
        ...
        if let selectedTabID = tabStore.selectedTabID,
           tabStore.tabs.contains(where: { $0.id == selectedTabID }) {
            return selectedTabID
        }

        return tabStore.tabs.first(where: { !$0.isPinned })?.id
    }
    ...
    private func handleFileChanged(tabID: UUID) {
        ...
        // Skip if content matches (e.g. user just saved from Scratchpad)
    ...
    private func applySettings() {
        ...
        let vimEnabled = settings.isVimModeEnabled
        ...
        for (_, state) in editorStates {
            ...
            if !vimEnabled {
                state.vimStatusLineView.stringValue = ""
                state.vimStatusLineView.isHidden = true
            }
            ...
            EditorStateFactory.applyTheme(
                ...
                vimStatusLine: state.vimStatusLineView
            )
        }
