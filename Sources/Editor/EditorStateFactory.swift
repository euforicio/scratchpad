...
struct EditorState {
    ...
    let vimStatusLineView: NSTextField
...
enum EditorStateFactory {
    ...
    static func create(for tab: TabData) -> EditorState {
        ...
        let vimStatusLine = createVimStatusLine()
        ...
        applyTheme(textView: textView, gutter: gutter, coordinator: highlighter, vimStatusLine: vimStatusLine)
        ...
        return EditorState(
            ...
            gutterView: gutter,
            vimStatusLineView: vimStatusLine,
    ...
    private static func createVimStatusLine() -> NSTextField {
        let statusLine = NSTextField(labelWithString: "")
        statusLine.translatesAutoresizingMaskIntoConstraints = false
        statusLine.isEditable = false
        statusLine.isBordered = false
        statusLine.isBezeled = false
        statusLine.isSelectable = false
        statusLine.drawsBackground = true
        statusLine.backgroundColor = NSColor.clear
        statusLine.textColor = NSColor.secondaryLabelColor
        statusLine.font = NSFont.monospacedSystemFont(ofSize: 12, weight: .medium)
        statusLine.lineBreakMode = .byTruncatingTail
        statusLine.maximumNumberOfLines = 1
        statusLine.alignment = .left
    ...
    static func applyTheme(textView: EditorTextView, gutter: LineNumberGutterView, coordinator: SyntaxHighlightCoordinator, vimStatusLine: NSTextField) {
        let settings = SettingsStore.shared
        ...
        vimStatusLine.textColor = isDark ? NSColor(white: 0.78, alpha: 1) : NSColor(white: 0.2, alpha: 1)
        vimStatusLine.backgroundColor = isDark
            ? NSColor(white: 0.15, alpha: 0.9)
            : NSColor(white: 0.9, alpha: 0.9)
        vimStatusLine.drawsBackground = true
    }
}
