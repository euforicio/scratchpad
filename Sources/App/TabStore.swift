...
class TabStore: ObservableObject {
    ...
    init(sessionURL: URL? = nil) {
        if let sessionURL {
            ...
            self.sessionURL = scratchpadDir.appendingPathComponent("session.json")
        }
    ...
    func addWelcomeTab() {
        let tab = TabData(
        name: String(localized: "welcome.tab_name", defaultValue: "Welcome to Scratchpad"),
    ...
    private func withSecurityScopedAccess<T>(to url: URL, _ block: () throws -> T) throws -> T {
    ...
    func saveFile(id: UUID) {
        ...
        if let fileURL = tabs[index].fileURL {
            do {
                ...
                NSLog("Failed to save file: \(error), url: \(fileURL.path)")
            }
        } else {
            saveFileAs(id: id)
        }
    ...
    func saveFileAs(id: UUID) {
        ...
        do {
            ...
            NSLog("Failed to save file: \(error), url: \(url.path)")
        }
    }
    ...
    func openFile(url: URL) {
        ...
        let content: String
        do {
            content = try withSecurityScopedAccess(to: url) {
                try String(contentsOf: url, encoding: .utf8)
            }
        } catch {
            NSLog("Failed to open file: \(error), url: \(url.path)")
            return
        }
    ...
    // lived at ~/Library/Application Support/Scratchpad/session.json. Starting with
    // 1.8.0 we enabled App Sandbox for App Store distribution, which moves the
    // data directory into ~/Library/Containers/io.euforic.scratchpad/Data/
    // Library/Application Support/Scratchpad/session.json.
    //
    ...
    //
    // Solution: the direct-download build (scratchpad-direct.entitlements) has a
    ...
    private static func migrateLegacyData(to sandboxedURL: URL) {
        ...
        let oldDir = "\(realHome)/Library/Application Support/Scratchpad"
