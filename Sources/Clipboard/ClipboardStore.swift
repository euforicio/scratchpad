...
class ClipboardStore {
    ...
    init(storageURL: URL? = nil, imagesDirectory: URL? = nil) {
        ...
        if let storageURL {
            ...
            self.storageURL = scratchpadDir.appendingPathComponent("clipboard.json")
        }

        if let imagesDirectory {
            self.imagesDirectory = imagesDirectory
        } else {
            self.imagesDirectory = scratchpadDir.appendingPathComponent("clipboard-images")
        }
    ...
    // non-sandboxed path (~/Library/Application Support/Scratchpad/) into the
