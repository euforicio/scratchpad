...
class SettingsStore: ObservableObject {
    ...
    @Published var syntaxTheme: String = "scratchpad" {
        didSet {
            guard !isLoading else { return }
    ...
    var showOnlyInMenuBar: Bool {
        get { showInMenuBar && !showInDock }
        set {
            if newValue {
                showInMenuBar = true
                showInDock = false
            } else if showInMenuBar && !showInDock {
                showInDock = true
            }
        }
    }
    ...
    @Published var isVimModeEnabled: Bool = false {
        didSet {
            guard !isLoading else { return }
            defaults.set(isVimModeEnabled, forKey: "isVimModeEnabled")
            NotificationCenter.default.post(name: .settingsChanged, object: nil)
        }
    }
    ...
    func setICloudSync(_ enabled: Bool) {
        ...
        defaults.synchronize()
        if enabled {
            if !CloudSyncEngine.shared.start() {
                isLoading = true
                icloudSync = false
                defaults.set(false, forKey: "icloudSync")
                isLoading = false
            }
        } else {
    ...
    private func loadSettings() {
        ...
        let savedTheme = defaults.string(forKey: "syntaxTheme") ?? "scratchpad"
        syntaxTheme = SyntaxThemeRegistry.themes.contains(where: { $0.id == savedTheme }) ? savedTheme : "scratchpad"
        ...
        isVimModeEnabled = defaults.object(forKey: "isVimModeEnabled") as? Bool ?? false
